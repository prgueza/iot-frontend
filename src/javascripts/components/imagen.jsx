...
export const Imagen = ({ imagen }) => {
    const tiempo = moment(imagen.fecha).from(ahora);
    const claseEtiquetas = cx(
      {"fa fa-tags fa-flip-horizontal mr-1": imagen.etiquetas.length > 1},
      {"fa fa-tag fa-flip-horizontal mr-1": imagen.etiquetas.length < 2}
    );
    const claseElemento = cx("list-group-item-action elemento-display list-group-item flex-column align-items-start");
    var location = {
      pathname: '/imagenes/' + imagen.identificacion
    }
    return(
        ...
        <NavLink to={location}>
          <div className="elemento">
            <div className="d-flex w-100 justify-content-between">
              <h5 className="mb-1">{imagen.nombre}</h5>
              <small><i className="fa fa-hashtag" aria-hidden="true"></i> {imagen.identificacion}</small>
            </div>
            <p className="mb-1">{imagen.descripcion}</p>
            <div className="d-flex w-100 justify-content-between">
              <small><i className={claseEtiquetas} aria-hidden="true"></i>{imagen.etiquetas.length}</small>
...
export const DetallesImagen = ({ elemento }) => {
  const fecha = moment(elemento.fecha).format("dddd, D [de] MMMM [de] YYYY");
  var key = 0;
  const etiquetas = elemento.etiquetas.map(( etiqueta ) => <Etiqueta key={key++} categoria='imagenes' etiqueta={etiqueta}/>);
  return(
  <div className="col">
        ...
        <div className="row">
          <div className="col-7">
            <div className="card-body">
              ...
              <p className="card-text"><i className="fa fa-file-image-o mr-1" aria-hidden="true"></i> {elemento.formato}</p>
              <p className="card-text"><i className="fa fa-database mr-1" aria-hidden="true"></i> {elemento.bytes}</p>
