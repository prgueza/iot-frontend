...
export const Grupo = ({ grupo }) => {
  const tiempo = moment(grupo.fecha).from(ahora);
  const claseEtiquetas = cx(
    {"fa fa-tags fa-flip-horizontal mr-1": grupo.etiquetas.length > 1},
    {"fa fa-tag fa-flip-horizontal mr-1": grupo.etiquetas.length < 2}
  );
  const claseElemento = cx("list-group-item-action elemento-display list-group-item flex-column align-items-start");
  var location = {
    pathname: '/grupos/' + grupo.identificacion
  }
  return(
      ...
      <NavLink to={location}>
        <div className="elemento">
          <div className="d-flex w-100 justify-content-between">
            ...
            <small><i className="fa fa-hashtag" aria-hidden="true"></i> {grupo.identificacion}</small>
          </div>
          <p className="mb-1">{grupo.descripcion}</p>
          <div className="d-flex w-100 justify-content-between">
            <small><i className={claseEtiquetas} aria-hidden="true"></i>{grupo.etiquetas.length}</small>
...
export const DetallesGrupo = ({ elemento }) => {
  const fecha = moment(elemento.fecha).format("dddd, D [de] MMMM [de] YYYY");
  var key = 0;
  const etiquetas = elemento.etiquetas.map(( etiqueta ) => <Etiqueta key={key++} categoria='grupos' etiqueta={etiqueta}/>);
  return(
  <div className="col">
