...
export class ImageForm extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      updated_at: moment(),
      displays: image
        ? image.displays.map((d) => d._id)
        : [],
      groups: image
        ? image.groups.map((g) => g._id)
        : [],
      color: image
        ? image.color_profile
        : 'color',
  ...
  handleSubmit = () => {
    ...
    this.state.displays.length > 0
      ? form.displays = this.state.displays
      : form.displays = [];
  ...
  render() {
    ...
    const optionsDisplays = displays.sort((a, b) => a.updated_at - b.updated_at).map((d) => <label key={d._id} className="custom-control custom-checkbox">
            ...
            <li className="nav-item mr-auto">
              {
                image
                  ? <h2 className="detalles-titulo">
                      <i className="fa fa-pencil mr-3" aria-hidden="true"></i>Editar una imagen</h2>
                  : <h2 className="detalles-titulo">
                      <i className="fa fa-plus-circle mr-3" aria-hidden="true"></i>Añadir una nueva imagen</h2>
              }
            </li>
            <li className="nav-item ml-2">
              {
                image
                  ? <button onClick={this.handleSubmit} type="button" className="btn btn-outline-info">
                      <i className="fa fa-save mr-2" aria-hidden="true"></i>Guardar cambios</button>
                  : <button onClick={this.handleSubmit} type="button" className="btn btn-outline-info">
                      <i className="fa fa-plus-circle mr-2" aria-hidden="true"></i>Añadir</button>
              }
            </li>
          </ul>
        </div>
              ...
              <div className="form-group col">
                {
                  this.state.tags.map(
                    (t, i) => t.length > 1
                    ? <button type="button" className="btn mr-1 btn-outline-imagen btn-tiny" key={i}>{t}</button>
                    : '')
                }
              </div>
            </div>
