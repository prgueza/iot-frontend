...
export class DisplayForm extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      category: display
        ...
        : '',
      created_at: display
        ? moment(display.created_at)
        : moment(),
      updated_at: moment(),
      active_image: display
        ? display.active_image
          ? display.active_image._id
          : ''
        : '',
      images: display
        ? display.images.map((i) => i._id)
        : [],
      groups: display
        ? display.groups.map((g) => g._id)
        : [],
      device: '',
      deviceDescription: '',
  ...
  handleInputChange = (event) => {
    ...
    if (name === 'device') {
      ...
      this.setState({
        deviceDescription: unusedDevices.find((d) => d._id == value).description
      })
    }
  ...
  handleCheckImages = (event) => {
    ...
    this.setState({
      optionsActiveImage: this.props.data.images.filter((i) => this.state.images.find((c) => c == i._id)).map((i) => <option value={i._id} key={i._id}>{i.name}</option>)
    })
  }
  ...
  handleSubmit = () => {
    ...
    if (!this.props.display) 
      form.created_by = this.props.user._id
    this.state.active_image != ''
      ? form.active_image = this.state.active_image
      : form.active_image = null
    this.state.images.length > 0
      ? form.images = this.state.images
      : form.images = []
    this.state.groups.length > 0
      ? form.groups = this.state.groups
      : form.groups = []
    // HTTP request
    axios({
      method: display
  ...
  render() {
    ...
    const optionsGroups = groups.map((g) => <label key={g._id} className="custom-control custom-checkbox">
    ...
    const optionsImages = images.sort((a, b) => a.updated_at - b.updated_at).map((i) => <label key={i._id} className="custom-control custom-checkbox">
        ...
        <div className="card-header">
          <ul className="nav nav-pills card-header-pills justify-content-end mx-1">
            ...
            <li className="nav-item ml-2">
              {
                display
                  ? <button onClick={this.handleSubmit} type="button" className="btn btn-outline-success">
                      <i className="fa fa-save mr-3" aria-hidden="true"></i>Guardar cambios</button>
                  : <button onClick={this.handleSubmit} type="button" className="btn btn-outline-success">
                      <i className="fa fa-plus-circle mr-3" aria-hidden="true"></i>Guardar configuración</button>
              }
            </li>
          </ul>
        </div>
            ...
            <div className="form-row">
              <div className="form-group col">
                <label htmlFor="category">
                  <i className="fa fa-arrows-alt mr-2"></i>Categoría</label>
              ...
              <div className="form-group col">
                <label htmlFor="active_image">
                  <i className="fa fa-picture-o mr-2"></i>Seleccionar la imagen activa</label>
            ...
            <div className="form-row">
              <div className="form-group col">
                <label htmlFor="displays">
                  <i className="fa fa-television mr-2"></i>Asociar una o varias imagenes</label>
              ...
              <div className="form-group col">
                <label htmlFor="groups">
                  <i className="fa fa-list mr-2"></i>Asociar a uno o varios grupos</label>
                <div className="custom-controls-stacked">
                  {optionsGroups}
                </div>
              </div>
              ...
              <div className="form-group col">
                {
                  this.state.tags.map(
                    (t, i) => t.length > 1
                    ? <button type="button" className="btn mr-1 btn-outline-display btn-tiny" key={i}>{t}</button>
                    : '')
                }
              </div>
            </div>
